<template>
  <q-page class="relative-position overflow-hidden">
    <div class="search-block">
      ...
      <p class="q-px-lg text-subtitle2 overflow-hidden" v-show="searchContent">
        Search for "{{ hiddenSearch }}":
      </p>
      <p
        class="q-px-lg text-body2 text-weight-light overflow-hidden text-center"
    ...
    >
      <p
        class="q-px-lg q-pt-xl text-cyan-6 text-h5 text-center overflow-hidden"
    ...
    filterQueet: function () {
      let temp = this.queets.filter(
        (x) =>
          x.content
            .toLowerCase()
            .indexOf(this.searchContent.trim().toLowerCase()) !== -1
      );

      return temp;
    },
    hiddenSearch: function () {
      if (this.searchContent.length >= 27) {
        return `${this.searchContent.slice(0, 27)}...`;
      }
